// pages/SelectionPage.jsx – moved from components/SelectionScreen.jsx
...
function DetectedCard({ result, selectedVersion, onVersionChange, onStart, onRetry }) {
  ...
  const cfg = getEmulatorConfig(result.type)

  return (
    <div className="flex flex-col gap-6 w-full max-w-md">
      {/* Emulator card */}
      <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg-card)] p-6 flex items-center gap-5">
        <div className={`w-16 h-16 rounded-2xl ${cfg.bg} border ${cfg.border} flex items-center justify-center text-3xl shrink-0`}>
          {cfg.emoji}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <h2 className="text-lg font-bold text-[var(--text-primary)]">{cfg.name} Detected</h2>
          ...
          {result.status === 'auto_selected' && (
            <p className="text-sm text-[var(--accent)] mt-1 font-medium">{result.selected}</p>
          )}
